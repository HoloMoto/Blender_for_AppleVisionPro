                                       ...
                                       >
{
  ...
  /* External pointer (Bluetooth mouse on visionOS / iPad). Kept separate from finger gestures. */
  UITouch *pointer_active_touch_;
  bool pointer_left_down_;
  bool pointer_right_down_;
  CGPoint last_pointer_location_;
  bool has_last_pointer_location_;
  bool prefer_indirect_pointer_;
...
- (void)handlePointerScroll:(UIPanGestureRecognizer *)sender;
- (BOOL)prefersIndirectPointerInput;
- (CGPoint)scaledPointForTouch:(UITouch *)touch;
- (void)emitPointerCursorMove:(CGPoint)point;
- (void)processPointerTouchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event;
- (void)processPointerTouchesMoved:(NSSet<UITouch *> *)touches;
- (void)processPointerTouchesEnded:(NSSet<UITouch *> *)touches cancelled:(BOOL)cancelled;
...
- (void)setSystemAndWindowIOS:(GHOST_SystemIOS *)sys windowIOS:(GHOST_WindowIOS *)win
{
  ...
  pointer_active_touch_ = nil;
  pointer_left_down_ = false;
  pointer_right_down_ = false;
  last_pointer_location_ = CGPointZero;
  has_last_pointer_location_ = false;
  prefer_indirect_pointer_ = false;
...
- (void)registerGestureRecognizers
{
  ...
  /* Finger and pencil only; external mice use the IndirectPointer touch path. */
  ...
  /* External Bluetooth / trackpad pointer (required on visionOS; GCMouse alone is unreliable). */
  ...
  /* Mouse wheel / trackpad scroll (scroll events, not touches). */
  pointer_scroll_gesture_recognizer = [[UIPanGestureRecognizer alloc]
      initWithTarget:self
              action:@selector(handlePointerScroll:)];
  pointer_scroll_gesture_recognizer.delegate = self;
  pointer_scroll_gesture_recognizer.cancelsTouchesInView = false;
  pointer_scroll_gesture_recognizer.maximumNumberOfTouches = 0;
  if (@available(iOS 13.4, *)) {
    pointer_scroll_gesture_recognizer.allowedScrollTypesMask = UIScrollTypeMaskAll;
  }
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch
{
  ...
  if (@available(iOS 13.4, *)) {
    if (touch.type == UITouchTypeIndirectPointer) {
      return gestureRecognizer == pointer_hover_gesture_recognizer;
    }
  }
  return YES;
}
...
{
  if (gestureRecognizer == pointer_scroll_gesture_recognizer) {
...
- (BOOL)prefersIndirectPointerInput
{
  return prefer_indirect_pointer_;
}
...
- (void)emitPointerCursorMove:(CGPoint)point
{
  last_pointer_location_ = point;
  has_last_pointer_location_ = true;
  CGPoint loc = point;
  UserInputEvent event_info(&loc, nullptr, nullptr, false);
...
- (void)processPointerTouchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  if (@available(iOS 13.4, *)) {
    ...
    for (UITouch *touch in touches) {
      if (touch.type != UITouchTypeIndirectPointer || touch.phase != UITouchPhaseBegan) {
        continue;
      }
      if (pointer_active_touch_ != nil) {
        continue;
      }

      prefer_indirect_pointer_ = true;
      pointer_active_touch_ = touch;
      CGPoint point = [self scaledPointForTouch:touch];

      UserInputEvent event_info(&point, nullptr, nullptr, false);
      event_info.add_event(UserInputEvent::EventTypes::CURSOR_MOVE);

      if (mask & UIEventButtonMaskSecondary) {
        pointer_right_down_ = true;
        [self generateUserInputEvents:event_info];
...
- (void)processPointerTouchesMoved:(NSSet<UITouch *> *)touches
{
  if (@available(iOS 13.4, *)) {
    ...
    for (UITouch *touch in touches) {
      if (touch != pointer_active_touch_ || touch.type != UITouchTypeIndirectPointer ||
          touch.phase != UITouchPhaseMoved)
      {
        continue;
      }
      [self emitPointerCursorMove:[self scaledPointForTouch:touch]];
      break;
    }
  }
}
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self processPointerTouchesBegan:touches withEvent:event];
...
- (void)touchesMoved:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self processPointerTouchesMoved:touches];
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self processPointerTouchesEnded:touches cancelled:NO];
  current_pencil_touch = nil;
  tablet_data = GHOST_TABLET_DATA_NONE;
}
...
- (void)touchesCancelled:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self processPointerTouchesEnded:touches cancelled:YES];
  current_pencil_touch = nil;
  tablet_data = GHOST_TABLET_DATA_NONE;
}
...
- (void)handlePointerHover:(GHOSTUIHoverGestureRecognizer *)sender
{
  if (pointer_active_touch_ != nil || pointer_left_down_ || pointer_right_down_) {
    return;
  }
  if (sender.state == UIGestureRecognizerStateBegan ||
      sender.state == UIGestureRecognizerStateChanged)
  {
    ...
    last_pointer_location_ = hover_point;
    has_last_pointer_location_ = true;
    [self emitPointerCursorMove:hover_point];
  }
}
...
- (void)handlePointerScroll:(UIPanGestureRecognizer *)sender
{
  if (@available(iOS 13.4, *)) {
    ...
    prefer_indirect_pointer_ = true;
    const CGPoint delta = [sender translationInView:window->getView()];
    [sender setTranslation:CGPointZero inView:window->getView()];

    if (CGPointEqualToPoint(delta, CGPointZero)) {
      return;
    }

    const CGFloat scale = [window->getView() contentScaleFactor];
    const CGSize native_size = window->getNativeWindowSize();
    const int32_t cursor_x = has_last_pointer_location_ ?
                                   (int32_t)last_pointer_location_.x :
                                   (int32_t)(native_size.width * 0.5f);
    const int32_t cursor_y = has_last_pointer_location_ ?
                                   (int32_t)last_pointer_location_.y :
                                   (int32_t)(native_size.height * 0.5f);
...
- (void)externalKeyboardChange:(NSNotification *)notification
{
  ...
  if (external_keyboard_connected && !onscreen_keyboard_active) {
    window->becomeHardwareInputResponder();
  }
}

- (BOOL)isOnscreenKeyboardActive
{
  return onscreen_keyboard_active;
}
...
- (void)setInputTargets:(GHOST_SystemIOS *)system
                 window:(GHOST_WindowIOS *)window
          ghostUIWindow:(GHOSTUIWindow *)ghostUIWindow;
- (void)becomeHardwareInputResponderIfAppropriate;
...
@implementation GHOST_IOSViewController
{
  ...
  GHOSTUIWindow *ghost_ui_window_;
  BOOL hardware_input_configured_;
  int32_t hardware_cursor_x_;
  int32_t hardware_cursor_y_;
  id application_active_observer_;
}
...
- (nonnull instancetype)initWithMetalKitView:(nonnull MTKView *)mtkView
{
  ...
  system_ios_ = nullptr;
  window_ios_ = nullptr;
  ghost_ui_window_ = nil;
  hardware_input_configured_ = NO;
  hardware_cursor_x_ = 0;
  hardware_cursor_y_ = 0;
  application_active_observer_ = nil;
  self = [super init];
          ...
          ghostUIWindow:(GHOSTUIWindow *)ghostUIWindow
{
  system_ios_ = system;
  window_ios_ = window;
  ghost_ui_window_ = ghostUIWindow;
}
...
- (BOOL)shouldRouteHardwareInput
{
  if (ghost_ui_window_ != nil && [ghost_ui_window_ isOnscreenKeyboardActive]) {
    return NO;
  }
  return system_ios_ != nullptr && window_ios_ != nullptr;
}

- (void)becomeHardwareInputResponderIfAppropriate
{
  if (![self shouldRouteHardwareInput]) {
    return;
  }
  if (![self becomeFirstResponder]) {
    IOS_INPUT_LOG(@"becomeFirstResponder failed for hardware input");
  }
}

- (void)dispatchUIKey:(UIKey *)key down:(BOOL)down repeat:(BOOL)repeat
{
  if (![self shouldRouteHardwareInput] || key == nil) {
    return;
  }

  system_ios_->pushHardwareModifierFlags(window_ios_, (uint32_t)key.modifierFlags);

  NSString *charsIgnoringModifiers = key.charactersIgnoringModifiers;
  unichar recv_char = charsIgnoringModifiers.length > 0 ?
                          [charsIgnoringModifiers characterAtIndex:0] :
                          0;
  const GHOST_TKey ghost_key = convertKeyFromHIDUsage((int)key.keyCode, recv_char);
  if (ghost_key == GHOST_kKeyUnknown) {
    return;
  }

  char utf8_buf[6] = {'\0', '\0', '\0', '\0', '\0', '\0'};
  if (down) {
    NSString *characters = key.characters;
    if (characters.length > 0) {
      NSData *converted = [characters dataUsingEncoding:NSUTF8StringEncoding];
      const size_t copy_len = MIN(converted.length, sizeof(utf8_buf) - 1);
      if (copy_len > 0) {
        memcpy(utf8_buf, converted.bytes, copy_len);
      }
    }
  }

  if ((ghost_key >= GHOST_kKeyLeftArrow && ghost_key <= GHOST_kKeyDownArrow) ||
      (ghost_key >= GHOST_kKeyF1 && ghost_key <= GHOST_kKeyF20))
  {
    utf8_buf[0] = '\0';
  }

  system_ios_->pushHardwareKeyEvent(window_ios_,
...
- (void)handleMouseDelta:(float)deltaX deltaY:(float)deltaY
{
  if (![self shouldRouteHardwareInput]) {
    return;
  }

  const float scale = window_ios_->getWindowScaleFactor();
  hardware_cursor_x_ += (int32_t)(deltaX * scale);
  hardware_cursor_y_ += (int32_t)(deltaY * scale);

  const CGSize native_size = window_ios_->getNativeWindowSize();
  if (native_size.width > 0.0f) {
    hardware_cursor_x_ = std::clamp(
        hardware_cursor_x_, 0, std::max(0, (int32_t)native_size.width - 1));
  }
  if (native_size.height > 0.0f) {
    hardware_cursor_y_ = std::clamp(
        hardware_cursor_y_, 0, std::max(0, (int32_t)native_size.height - 1));
  }

  system_ios_->pushHardwareCursorMove(window_ios_, hardware_cursor_x_, hardware_cursor_y_);
}
...
- (void)configureMouse:(GCMouse *)mouse API_AVAILABLE(ios(14.0))
{
  ...
  /* Manual reference counting build: do not use __weak here. */
  GHOST_IOSViewController *block_self = self;
  input.mouseMovedHandler = ^(GCMouseInput *mouseInput, float deltaX, float deltaY) {
    if (block_self->ghost_ui_window_ != nil &&
        [block_self->ghost_ui_window_ prefersIndirectPointerInput])
    {
      return;
    }
    [block_self handleMouseDelta:deltaX deltaY:deltaY];
  };

  input.leftButton.pressedChangedHandler = ^(GCControllerButtonInput *button,
                                             float value,
                                             BOOL pressed) {
    if (![block_self shouldRouteHardwareInput]) {
      return;
    }
    if (block_self->ghost_ui_window_ != nil &&
        [block_self->ghost_ui_window_ prefersIndirectPointerInput])
    ...
    block_self->system_ios_->pushHardwareButtonEvent(block_self->window_ios_,
                                                     pressed ? GHOST_kEventButtonDown :
                                                               GHOST_kEventButtonUp,
                                                     GHOST_kButtonMaskLeft);
  };

  input.rightButton.pressedChangedHandler = ^(GCControllerButtonInput *button,
                                              float value,
                                              BOOL pressed) {
    if (![block_self shouldRouteHardwareInput]) {
      return;
  ...
  input.middleButton.pressedChangedHandler = ^(GCControllerButtonInput *button,
                                               float value,
                                               BOOL pressed) {
    if (![block_self shouldRouteHardwareInput]) {
      return;
...
- (void)configureConnectedMice API_AVAILABLE(ios(14.0))
{
...
{
  if (![notification.object isKindOfClass:[GCMouse class]]) {
    return;
  }
  [self configureMouse:(GCMouse *)notification.object];
  [self becomeHardwareInputResponderIfAppropriate];
}
...
- (void)applicationDidBecomeActive:(NSNotification *)notification
{
  ...
  if (@available(iOS 14.0, *)) {
    [self configureConnectedMice];
  }
  [self becomeHardwareInputResponderIfAppropriate];
}

- (void)setupHardwareInputIfNeeded
{
  if (hardware_input_configured_) {
    return;
  }
  hardware_input_configured_ = YES;

  const CGSize native_size = window_ios_ ? window_ios_->getNativeWindowSize() : CGSizeZero;
  hardware_cursor_x_ = native_size.width > 0.0f ? (int32_t)(native_size.width * 0.5f) : 0;
  hardware_cursor_y_ = native_size.height > 0.0f ? (int32_t)(native_size.height * 0.5f) : 0;
...
- (void)dealloc
{
  if (application_active_observer_ != nil) {
...
- (void)pressesBegan:(NSSet<UIPress *> *)presses withEvent:(UIPressesEvent *)event
{
  if ([self shouldRouteHardwareInput]) {
    for (UIPress *press in presses) {
      if (@available(iOS 13.4, *)) {
        if (press.key != nil) {
          /* UIEvent.keyRepeat is not exposed on all iOS SDK targets; repeat is optional. */
          [self dispatchUIKey:press.key down:YES repeat:NO];
        }
      }
    }
  }
...
- (void)pressesEnded:(NSSet<UIPress *> *)presses withEvent:(UIPressesEvent *)event
{
  if ([self shouldRouteHardwareInput]) {
    for (UIPress *press in presses) {
      if (@available(iOS 13.4, *)) {
        if (press.key != nil) {
          [self dispatchUIKey:press.key down:NO repeat:NO];
        }
      }
    }
  }
...
- (void)viewDidAppear:(BOOL)animated
{
  ...
  /* Defer Bluetooth keyboard / mouse setup until the view hierarchy is live (startup-safe). */
  if (window_ios_ != nullptr) {
    [self setupHardwareInputIfNeeded];
    [self becomeHardwareInputResponderIfAppropriate];
  }
}
    ...
    : GHOST_Window(width, height, state, context_params, false), metal_view_(nil)
{
  ...
  if (@available(iOS 13.0, *)) {
    UIWindowScene *window_scene = GHOST_IOS_GetActiveWindowScene();
    if (window_scene == nil) {
  ...
  [(GHOST_IOSViewController *)uiview_controller_ setInputTargets:system_ios_
                                                       window:this
                                                ghostUIWindow:ghost_rootWindow];
...
bool GHOST_WindowIOS::makeKeyWindow()
{
  ...
  if (is_active_window_ && system_ios_->current_active_window_ == this && !metal_view_.paused) {
    request_to_make_active_ = false;
    return true;
  }
...
#else
  /* iOS lifecycle callbacks can reactivate a window without a prior request flag. */
...
void GHOST_WindowIOS::becomeHardwareInputResponder()
{
  if (uiview_controller_ == nullptr) {
    return;
  }
  [(GHOST_IOSViewController *)uiview_controller_ becomeHardwareInputResponderIfAppropriate];
}
