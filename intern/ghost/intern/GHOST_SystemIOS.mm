...
static void ghost_ios_reactivate_window()
{
  ...
  if (system->next_active_window_ != nullptr) {
    ...
    system->current_active_window_ = system->next_active_window_;
    system->next_active_window_ = nullptr;
  }

  /* Hard fallback: reacquire focus for any live GHOST UIWindow instances directly through UIKit.
   * This helps normal app relaunches where internal active-window bookkeeping may be stale. */
...
{
  /* Startup/foreground race mitigation:
   * run immediate + delayed retries so responder/key-window acquisition survives
   * fast standalone launches (without debugger timing overhead). */
...
{
  NSLog(@"[Blender iOS] didFinishLaunching begin");
  ...
  /* Avoid blocking launch path with heavy initialization. */
  dispatch_async(dispatch_get_main_queue(), ^{
    NSLog(@"[Blender iOS] main_ios_callback begin");
    main_ios_callback(argc, argv);
    NSLog(@"[Blender iOS] main_ios_callback end");
    ghost_ios_schedule_reactivation_burst();
  });
...
GHOST_SystemIOS::GHOST_SystemIOS()
{
  ...
  input_reactivation_frames_left_ = 0;
