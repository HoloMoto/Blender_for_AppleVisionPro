...
typedef void (*GHOST_IOSFilePickerCallback)(const char *path, bool cancelled, void *user_data);

/** Present the system document picker to open a .blend (or other) file from the Files app. */
extern bool GHOST_IOS_present_open_document_picker(GHOST_IOSFilePickerCallback callback,
                                                   void *user_data);
/** Export a local .blend via the system save sheet (Files app / iCloud). */
extern bool GHOST_IOS_present_export_document_picker(const char *local_blend_path,
                                                     GHOST_IOSFilePickerCallback callback,
                                                     void *user_data);
/**
 * Copy a staged export file to the user-picked destination (or Documents/Exports fallback).
 * \param r_final_path: Buffer filled with the path of the exported file on success.
 */
extern bool GHOST_IOS_commit_export_file(const char *staged_path,
