...
Icon_Geom *BKE_icon_geom_from_memory(uchar *data, size_t data_len)
{
  ...
  if (memcmp(data, header, ARRAY_SIZE(header)) != 0) {
    MEM_freeN(data);
    return nullptr;
  }
...
Icon_Geom *BKE_icon_geom_placeholder_get(void)
{
  static Icon_Geom geom = {};
  static uchar coords[][2] = {{2, 2}, {14, 2}, {8, 14}};
  static uchar colors[][4] = {{200, 200, 200, 255},
                              {200, 200, 200, 255},
                              {200, 200, 200, 255}};

  if (geom.coords == nullptr) {
    geom.coords_len = 1;
    geom.coords_range[0] = 16;
    geom.coords_range[1] = 16;
    geom.coords = coords;
    geom.colors = colors;
    geom.mem = nullptr;
    geom.icon_id = 0;
...
Icon_Geom *BKE_icon_geom_from_file(const char *filename)
{
  ...
  if (BLI_file_data_is_git_lfs_pointer(data, data_len)) {
#if defined(__APPLE__) && defined(TARGET_OS_IPHONE) && TARGET_OS_IPHONE
    fprintf(stderr,
            "[ios] icon \"%s\" is a Git LFS pointer — run tools/ios_fetch_datafiles.sh and "
            "rebuild for real tool icons\n",
            filename);
    fflush(stderr);
#endif
    MEM_freeN(data);
