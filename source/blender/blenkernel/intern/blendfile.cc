...
BlendFileData *BKE_blendfile_read_from_memory(const void *file_buf,
                                              ...
                                              ReportList *reports)
{
  if (file_buf != nullptr && BLI_file_data_is_git_lfs_pointer(file_buf, size_t(file_buf_size))) {
    CLOG_ERROR(&LOG_BLEND,
               "Blend data is a Git LFS pointer (not a .blend file). "
               "Run `git lfs pull` in the Blender source tree, then rebuild.");
