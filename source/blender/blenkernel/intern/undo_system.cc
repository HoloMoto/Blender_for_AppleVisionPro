...
{
  /* Mode undo polls typically use #CTX_data_scene / #CTX_data_view_layer (often via
   * #BKE_view_layer_synced_ensure). Those are not guaranteed during early startup, e.g.
   * #wm_file_read_post before the window/screen context is fully wired. */
  Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = (scene != nullptr) ? CTX_data_view_layer(C) : nullptr;
  const bool mode_polls_safe = (scene != nullptr && view_layer != nullptr);

  LISTBASE_FOREACH (const UndoType *, ut, &g_undo_types) {
    /* No poll means we don't check context (also never matched here, see original behavior). */
    if (ut->poll == nullptr) {
      continue;
    }
    if (!mode_polls_safe) {
