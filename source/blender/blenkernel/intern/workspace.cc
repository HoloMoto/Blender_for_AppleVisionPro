...
void BKE_workspace_active_layout_ensure_for_window(const Main *bmain,
                                                   WorkSpaceInstanceHook *hook,
                                                   const int winid,
                                                   WorkSpace *workspace)
{
  if (hook == nullptr || workspace == nullptr || bmain == nullptr) {
    return;
  }
  ...
  if (layout == nullptr && !BLI_listbase_is_empty(&bmain->screens)) {
    layout = workspace_layout_find_exec(workspace, static_cast<bScreen *>(bmain->screens.first));
  }

  if (layout == nullptr) {
    for (WorkSpace *ws = static_cast<WorkSpace *>(bmain->workspaces.first); ws;
         ws = static_cast<WorkSpace *>(ws->id.next))
    {
      WorkSpaceLayout *wl = static_cast<WorkSpaceLayout *>(ws->layouts.first);
      if (wl != nullptr) {
        workspace = ws;
...
WorkSpaceInstanceHook *BKE_workspace_instance_hook_create(const Main *bmain, const int winid)
{
  ...
  /* Register each workspace's default layout for this window without passing null layouts: that
   * used to clear #WorkSpaceInstanceHook.act_layout and broke #WM_window_get_active_screen. */
  for (WorkSpace *workspace = static_cast<WorkSpace *>(bmain->workspaces.first); workspace;
       workspace = static_cast<WorkSpace *>(workspace->id.next))
  {
    WorkSpaceLayout *layout = static_cast<WorkSpaceLayout *>(workspace->layouts.first);
    if (layout != nullptr) {
      BKE_workspace_active_layout_set(hook, winid, workspace, layout);
    }
  }

  WorkSpace *ws = static_cast<WorkSpace *>(bmain->workspaces.first);
  if (ws != nullptr) {
    BKE_workspace_active_set(hook, ws);
    BKE_workspace_active_layout_ensure_for_window(bmain, hook, winid, ws);
  }
...
void BKE_workspace_instance_hook_free(const Main *bmain, WorkSpaceInstanceHook *hook)
{
  ...
  /* Workspaces should normally outlive WM. During early startup / failed file-read recovery there
   * may temporarily be no workspaces even in foreground; treat this as a valid no-op cleanup path.
   */
