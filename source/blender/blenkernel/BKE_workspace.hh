...
/**
 * Repair `hook->act_layout` (and `hook->active` when switching workspace) for \a workspace after
 * startup, file read, or #BKE_workspace_instance_hook_create when relations are incomplete.
 */
void BKE_workspace_active_layout_ensure_for_window(const Main *bmain,
                                                   WorkSpaceInstanceHook *hook,
                                                   int winid,
                                                   WorkSpace *workspace) ATTR_NONNULL(1, 2);
