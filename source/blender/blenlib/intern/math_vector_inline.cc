...
{
  /* Startup on some mobile paths can pass an invalid small address (e.g. `0x14`) before
   * event/window state is fully initialized. Keep this helper defensive so release builds
   * avoid crashing and can continue initialization. */
  if (UNLIKELY(a == nullptr || uintptr_t(a) < 4096)) {
    r[0] = 0;
    r[1] = 0;
    return;
  }
