...
BLI_INLINE bool BLI_listbase_is_single(const ListBase *lb)
{
  ...
  /* iOS: avoid faults on low-address garbage (incomplete context / null-base offset). */
  if (reinterpret_cast<uintptr_t>(lb) < 0x100000) {
    return false;
  }
#endif
...
{
#if defined(__APPLE__) && defined(TARGET_OS_IPHONE) && TARGET_OS_IPHONE
  /* iOS: avoid fault on `&((wmWindowManager *)nullptr)->windows` (e.g. ~0x210) during
   * incomplete context; valid heap pointers are never in the low user range on 64-bit. */
  if (reinterpret_cast<uintptr_t>(lb) < 0x100000) {
    return true;
  }
#endif
