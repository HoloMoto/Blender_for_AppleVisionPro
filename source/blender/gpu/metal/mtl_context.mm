...
void MTLContext::activate()
{
  ...
  /* iOS draw/bootstrap can re-enter activation on the same thread. */
  if (is_active_ && pthread_equal(thread_, pthread_self())) {
    return;
  }
#endif
...
id<MTLRenderCommandEncoder> MTLContext::ensure_begin_render_pass()
{
  ...
  if (!this->active_fb) {
#if defined(WITH_APPLE_CROSSPLATFORM)
    MTL_LOG_WARNING("No framebuffer is bound, restoring default framebuffer");
             ...
             id<CAMetalDrawable> drawable)
{
  ...
  /* Wait until there is room for one more in-flight present. Using `>=` (not `>`) is required:
   * with `perf_max_drawables == 1`, `count == 1` must block the next frame until the completion
   * handler runs; otherwise we could increment to 2 and spin forever if a buffer never completes
   * (iOS observed: main thread stuck, MTKView heartbeats stop). */
  const int drawable_limit = min_ii(perf_max_drawables, MTL_MAX_DRAWABLES);
  uint32_t drawable_wait_ms = 0;
  while (MTLContext::max_drawables_in_flight >= drawable_limit) {
    BLI_time_sleep_ms(1);
    drawable_wait_ms++;
    if (drawable_wait_ms > 2000) {
      const int stuck_count = MTLContext::max_drawables_in_flight.load();
      MTL_LOG_WARNING(
          "Present wait timeout (%u ms). in_flight=%d limit=%d. Forcing recovery to avoid "
          "main-thread stall.",
          drawable_wait_ms,
          stuck_count,
          drawable_limit);
      MTLContext::max_drawables_in_flight.store(max_ii(0, drawable_limit - 1));
      break;
    }
  }
