...
bool supports_barycentric_whitelist(id<MTLDevice> device)
{
  ...
  /* Verify GPU support (Mac uses discrete/Metal2.2 table; iOS uses Apple GPU families). */
#if defined(WITH_APPLE_CROSSPLATFORM) && TARGET_OS_IPHONE
  bool supported_gpu = [device supportsFamily:MTLGPUFamilyApple4];
#else
...
#if defined(WITH_APPLE_CROSSPLATFORM) && TARGET_OS_IPHONE
  /* #MTLGPUFamilyMac2 is for Mac discrete GPUs; iOS devices use Apple GPU families. */
  bool supported_metal_version = [device supportsFamily:MTLGPUFamilyApple4];
#else
