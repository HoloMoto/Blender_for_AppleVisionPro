...
/** Ensure `CTX_data_main` / `CTX_wm_manager` match `G_MAIN` when stale (embedded platforms). */
...
void WM_redraw_windows(bContext *C);
    ...
    TARGET_OS_IPHONE
/**
 * Force all visible regions on the active screen to draw on iOS.
 * Needed when opening fullscreen temp editors (preferences, file browser) on a single-surface
 * platform where notifier-driven redraw may not schedule a frame.
 */
