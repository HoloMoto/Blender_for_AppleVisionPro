...
{
  if (platform_support_key == nullptr || platform_support_key[0] == '\0') {
    return false;
  }
...
bool WM_platform_support_perform_checks()
{
  ...
  /* iOS startup path can reach here before GPU platform info is fully initialized.
   * Skip desktop-oriented GPU support dialogs/checks to avoid startup asserts. */
  return true;
#endif
