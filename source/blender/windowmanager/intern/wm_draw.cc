...
static void wm_draw_window(bContext *C, wmWindow *win)
{
  ...
  /* #screen_refresh_if_needed only runs when #bScreen.do_refresh is set. It is valid for the
   * screen to have #do_draw (from the tail of a full refresh) while no #ARegion has `do_draw` yet
   * — #ED_screen_ensure_updated then no-ops and we clear the screen flag at the end of this
   * function without ever painting a region (infinite gray on Metal). */
  if (!G.background && screen != nullptr && screen->do_draw) {
    auto ios_any_region_wants_draw = [&]() -> bool {
      LISTBASE_FOREACH (ARegion *, region, &screen->regionbase) {
        if (region->runtime->visible && region->runtime->do_draw) {
          return true;
        }
    ...
    if (!ios_any_region_wants_draw()) {
      ...
      if (!ios_any_region_wants_draw()) {
        LISTBASE_FOREACH (ARegion *, region, &screen->regionbase) {
          if (region->runtime->visible && (region->flag & RGN_FLAG_POLL_FAILED) == 0) {
...
static bool wm_draw_update_test_window(Main *bmain, bContext *C, wmWindow *win)
{
  ...
  bool do_draw = false;
#if defined(__APPLE__) && defined(TARGET_OS_IPHONE) && TARGET_OS_IPHONE
  static uint64_t ios_draw_probe_counter = 0;
  ios_draw_probe_counter++;
#endif

  /* iOS / early startup: active screen may not be wired yet (see WM_window_get_active_screen). */
  if (screen == nullptr) {
#if defined(__APPLE__) && defined(TARGET_OS_IPHONE) && TARGET_OS_IPHONE
    if ((ios_draw_probe_counter % 300) == 0) {
      fprintf(stderr,
              "[ios][draw_probe] no active screen win=%p scene=%p view_layer=%p depsgraph=%p\n",
              (void *)win,
              (void *)scene,
              (void *)view_layer,
              (void *)depsgraph);
  ...
  int drawable_regions = 0;
  int hidden_regions = 0;
  int poll_failed_regions = 0;
  int too_small_regions = 0;
  LISTBASE_FOREACH (ARegion *, region, &screen->regionbase) {
    ...
    if (region->runtime->visible) {
      ...
      if (region->flag & RGN_FLAG_TOO_SMALL) {
        too_small_regions++;
      }
    }
    if (region->runtime->visible && region->runtime->do_draw) {
...
#if defined(__APPLE__) && defined(TARGET_OS_IPHONE) && TARGET_OS_IPHONE
  /* If all regions stay invisible, force periodic refresh to rebuild area/region layout.
   * Without this, startup can stay in a gray heartbeat loop forever. */
  if (visible_regions == 0 && area_count > 0) {
    if ((ios_draw_probe_counter % 60) == 0) {
      screen->do_refresh = true;
      screen->do_draw = true;
      fprintf(stderr,
              "[ios][draw_probe] forcing refresh win=%p screen=%p areas=%d scene=%p view_layer=%p "
              "hidden=%d poll_failed=%d too_small=%d\n",
              (void *)win,
              (void *)screen,
              area_count,
              (void *)scene,
  ...
  if ((ios_draw_probe_counter % 300) == 0) {
    fprintf(stderr,
            "[ios][draw_probe] no redraw win=%p screen=%p areas=%d vis_regions=%d draw_regions=%d "
            "hidden=%d poll_failed=%d too_small=%d scene=%p view_layer=%p "
            "flags(refresh=%d draw=%d gesture=%d paint=%d drag=%d)\n",
            (void *)win,
            (void *)screen,
            area_count,
            visible_regions,
            ...
            (void *)scene,
            (void *)view_layer,
            int(screen->do_refresh),
            int(screen->do_draw),
            int(screen->do_draw_gesture),
            int(screen->do_draw_paintcursor),
            int(screen->do_draw_drag));
    fflush(stderr);

    if (drawable_regions == 0 && visible_regions > 0) {
      auto ios_log_region_state = [&](ARegion *region, const char *scope, int area_index) {
        fprintf(stderr,
                "[ios][draw_probe][region] %s area=%d type=%d align=%d vis=%d do_draw=%d "
                "poll_failed=%d too_small=%d winrct=(%d,%d)-(%d,%d)\n",
                scope,
                area_index,
                int(region->regiontype),
                int(region->alignment),
                int(region->runtime->visible),
                int(region->runtime->do_draw),
                int((region->flag & RGN_FLAG_POLL_FAILED) != 0),
                int((region->flag & RGN_FLAG_TOO_SMALL) != 0),
                region->winrct.xmin,
                region->winrct.ymin,
                region->winrct.xmax,
                region->winrct.ymax);
      ...
      LISTBASE_FOREACH (ARegion *, region, &screen->regionbase) {
        ios_log_region_state(region, "screen", -1);
      }
      int area_index = 0;
      ED_screen_areas_iter (win, screen, area) {
        LISTBASE_FOREACH (ARegion *, region, &area->regionbase) {
          ios_log_region_state(region, "area", area_index);
        }
        area_index++;
      }
      fflush(stderr);
  ...
  /* Keep trying to bootstrap redraw when startup gets stuck in a gray heartbeat.
   * A bounded streak avoids an infinite forced redraw loop once UI becomes healthy. */
  static int ios_no_redraw_streak = 0;
  if (drawable_regions > 0) {
    ios_no_redraw_streak = 0;
  }
  else if (area_count > 0 && visible_regions > 0 && !screen->do_refresh && !screen->do_draw &&
           !screen->do_draw_gesture && !screen->do_draw_paintcursor && !screen->do_draw_drag)
  {
    ios_no_redraw_streak++;
    if (ios_no_redraw_streak > 600 && (ios_no_redraw_streak % 120) != 0) {
      return false;
    }

    LISTBASE_FOREACH (ARegion *, region, &screen->regionbase) {
      if (region->runtime->visible && (region->flag & RGN_FLAG_POLL_FAILED) == 0 &&
          (region->flag & RGN_FLAG_TOO_SMALL) == 0)
      {
        ED_region_tag_redraw(region);
        /* iOS startup can miss notifier-driven redraw tags indefinitely; force at least one
         * drawable visible region so #wm_draw_window_offscreen executes real region draws. */
    ...
    screen->do_refresh = true;
    if ((ios_no_redraw_streak % 120) == 1) {
      fprintf(stderr,
              "[ios][draw_probe] bootstrap redraw streak=%d win=%p screen=%p vis=%d draw=%d\n",
              ios_no_redraw_streak,
              (void *)win,
              (void *)screen,
              visible_regions,
...
#if defined(WITH_APPLE_CROSSPLATFORM)
  if (BLI_listbase_count(&wm->windows) > 1 && wm_ios_temp_overlay_window_get(wm) == nullptr) {
    wm_ios_ensure_single_window(C, wm, false);
  }
#endif

#if defined(WITH_APPLE_CROSSPLATFORM) || \
    (defined(__APPLE__) && defined(TARGET_OS_IPHONE) && TARGET_OS_IPHONE)
  /* First Metal frames can run before workspace/screen wiring is complete; re-run the same init
   * path as startup so drawing is not skipped indefinitely (#WM_window_get_active_screen). */
  {
    bool missing_active_screen = false;
    LISTBASE_FOREACH (wmWindow *, win_iter, &wm->windows) {
      if (WM_window_get_active_screen(win_iter) == nullptr) {
        missing_active_screen = true;
        break;
      }
    }
    if (missing_active_screen) {
...
#if defined(WITH_APPLE_CROSSPLATFORM)
  wmWindow *ios_temp_overlay = wm_ios_temp_overlay_window_get(wm);
#endif
...
#if defined(WITH_APPLE_CROSSPLATFORM)
    /* Preferences and other temp overlays share the primary GHOST surface. */
    if (ios_temp_overlay != nullptr && win->parent == nullptr && win != ios_temp_overlay) {
      continue;
    }
#endif
...
#if defined(__APPLE__) && defined(TARGET_OS_IPHONE) && TARGET_OS_IPHONE
      /* #ED_screen_ensure_updated can clear per-region #do_draw without scheduling another pass. */
      {
        bScreen *ios_screen = WM_window_get_active_screen(win);
        if (ios_screen != nullptr) {
          bool tagged = false;
          LISTBASE_FOREACH (ARegion *, region, &ios_screen->regionbase) {
            if (region->runtime->visible && (region->flag & RGN_FLAG_POLL_FAILED) == 0) {
              ED_region_tag_redraw(region);
              tagged = true;
            }
          }
          ED_screen_areas_iter (win, ios_screen, area) {
            LISTBASE_FOREACH (ARegion *, region, &area->regionbase) {
              if (region->runtime->visible && (region->flag & RGN_FLAG_POLL_FAILED) == 0) {
                ED_region_tag_redraw(region);
                tagged = true;
...
#if defined(__APPLE__) && defined(TARGET_OS_IPHONE) && TARGET_OS_IPHONE
void WM_ios_force_screen_redraw(bContext *C)
{
  wmWindow *win = CTX_wm_window(C);
  if (win == nullptr) {
    return;
  ...
  if (win->ghostwin) {
    GHOST_InvalidateWindow(static_cast<GHOST_WindowHandle>(win->ghostwin));
  }
