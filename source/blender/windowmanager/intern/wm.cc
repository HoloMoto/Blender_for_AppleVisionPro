...
void WM_check(bContext *C)
{
  ...
  if (bmain == nullptr) {
    /* Startup on some embedded paths can lag `CTX_data_main` behind `G_MAIN`. */
    bmain = G_MAIN;
    if (bmain != nullptr) {
  ...
  if (BLI_listbase_is_empty(&wm->windows)) {
    /* Startup/file-read recovery on mobile can temporarily leave WM without windows.
     * Create a default one so the app can progress past black screen. */
...
{
  Main *bmain_for_wm = CTX_data_main(C);
  if (bmain_for_wm == nullptr) {
    return;
  }
  wmWindowManager *wm = CTX_wm_manager(C);
  ...
  /* WM context. */
  if (wm == nullptr) {
    wm = static_cast<wmWindowManager *>(bmain_for_wm->wm.first);
    CTX_wm_manager_set(C, wm);
  }
...
void wm_context_ensure_from_main(bContext *C)
{
  ...
  /* iOS: single global Main; always match #G_MAIN so UIKit/Metal never keeps a stale #CTX_wm_manager
   * (CMake may not define #WITH_APPLE_CROSSPLATFORM on every translation unit). */
  if (G_MAIN != nullptr) {
...
void wm_add_default(Main *bmain, bContext *C)
{
  ...
  /* Context screen may be unset; startup data may be incomplete on some platforms. Fall back to the
   * first workspace layout so #BKE_workspace_active_layout_set never receives a null workspace. */
  if ((workspace == nullptr || layout == nullptr) &&
      !BLI_listbase_is_empty(&bmain->workspaces))
  {
    workspace = static_cast<WorkSpace *>(bmain->workspaces.first);
    layout = static_cast<WorkSpaceLayout *>(workspace->layouts.first);
  }
  if (workspace != nullptr && layout == nullptr && !BLI_listbase_is_empty(&bmain->screens)) {
    layout = BKE_workspace_layout_find(workspace, static_cast<bScreen *>(bmain->screens.first));
  }
  if (screen == nullptr && layout != nullptr) {
    screen = layout->screen;
  }

  /* Allocate runtime before touching `wm->runtime->...` (must match blend-read path). */
  BLI_assert(wm->runtime == nullptr);
  wm->runtime = MEM_new<blender::bke::WindowManagerRuntime>(__func__);
  ...
  if (scene == nullptr) {
    /* Context scene may not be set yet during startup / file read finalize (e.g. mobile). */
