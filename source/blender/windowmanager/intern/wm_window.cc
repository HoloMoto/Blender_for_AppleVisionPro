...
static bool wm_ios_ghostwin_is_shared(const wmWindow *win)
{
  return win->parent != nullptr && win->ghostwin != nullptr &&
         win->ghostwin == win->parent->ghostwin;
}

static void wm_ios_share_parent_surface(wmWindow *win)
{
  wmWindow *parent = win->parent;
  BLI_assert(parent != nullptr && parent->ghostwin != nullptr);

  win->ghostwin = parent->ghostwin;
  win->gpuctx = parent->gpuctx;
  win->sizex = parent->sizex;
  win->sizey = parent->sizey;
  win->posx = parent->posx;
  win->posy = parent->posy;
}

wmWindow *wm_ios_temp_overlay_window_get(wmWindowManager *wm)
{
  if (wm == nullptr) {
    return nullptr;
  }
  LISTBASE_FOREACH (wmWindow *, win, &wm->windows) {
    if (win->parent == nullptr || win->ghostwin == nullptr) {
      continue;
    }
...
#if defined(WITH_APPLE_CROSSPLATFORM)
  if (wm_ios_ghostwin_is_shared(win)) {
...
void wm_window_close(bContext *C, wmWindowManager *wm, wmWindow *win)
{
  ...
  if (win->parent == nullptr && win_other == nullptr) {
    /* During startup/file-read recovery, invalid windows may be removed before a valid GHOST
     * window/context exists. In that state, opening a quit-confirm popup can recurse into
     * half-initialized UI paths and stall startup. */
    if (win->ghostwin != nullptr && CTX_data_main(C) != nullptr) {
  ...
  if ((screen && screen->temp) || is_single_editor) {
    ...
    if (workspace != nullptr && layout != nullptr) {
...
static void wm_window_ghostwindow_ensure(wmWindowManager *wm, wmWindow *win, bool is_dialog)
{
  if (win->ghostwin == nullptr) {
#if defined(WITH_APPLE_CROSSPLATFORM)
    if (win->parent != nullptr && win->parent->ghostwin != nullptr) {
      wm_ios_share_parent_surface(win);
    }
    else
#endif
    {
...
#if defined(WITH_APPLE_CROSSPLATFORM)
void wm_ios_ensure_single_window(bContext *C, wmWindowManager *wm, const bool relayout)
{
  ...
  Main *bmain = CTX_data_main(C);
  /* Keep the window that already owns the GHOST surface (file-read can reorder wm->windows). */
  wmWindow *primary = static_cast<wmWindow *>(wm->windows.first);
  LISTBASE_FOREACH (wmWindow *, win, &wm->windows) {
    if (win->parent == nullptr && win->ghostwin != nullptr) {
      primary = win;
      break;
  ...
  bool closed_extra = false;
  LISTBASE_FOREACH_MUTABLE (wmWindow *, win, &wm->windows) {
    if (win != primary && win->parent == nullptr) {
      /* Keep the sole GHOST surface on the primary wmWindow. */
      if (primary->ghostwin == nullptr && win->ghostwin != nullptr) {
        primary->ghostwin = win->ghostwin;
        primary->gpuctx = win->gpuctx;
        GHOST_SetWindowUserData(static_cast<GHOST_WindowHandle>(primary->ghostwin), primary);
        win->ghostwin = nullptr;
        win->gpuctx = nullptr;
      }
      fprintf(stderr, "[ios] closing extra window win=%p (primary=%p)\n", (void *)win, (void *)primary);
      fflush(stderr);
      wm_window_close(C, wm, win);
      closed_extra = true;
    }
  ...
  if (!closed_extra && !relayout) {
    return;
  }

  CTX_wm_window_set(C, primary);
  ED_screens_init(C, bmain, wm);
  if (relayout) {
    ED_screens_ios_ensure_sidebar_editors(C, bmain, primary);
    ED_screens_ios_activate_layout_workspace(C, bmain, primary);
  }
  ED_screen_refresh(C, wm, primary);
  wm_window_make_drawable(wm, primary);

  if (primary->ghostwin) {
    GHOST_InvalidateWindow(static_cast<GHOST_WindowHandle>(primary->ghostwin));
  }
}
#endif /* WITH_APPLE_CROSSPLATFORM */
...
#if defined(WITH_APPLE_CROSSPLATFORM)
  /* Single-window platform: never spawn another top-level GHOST window (breaks layout/input). */
  if (!G.background && toplevel && !dialog && !temp && !BLI_listbase_is_empty(&wm->windows)) {
    wmWindow *primary = static_cast<wmWindow *>(wm->windows.first);
    fprintf(stderr,
            "[ios] WM_window_open blocked toplevel, reusing primary win=%p\n",
            (void *)primary);
    fflush(stderr);
    return primary;
  }
...
static bool ghost_event_proc(GHOST_EventHandle ghost_event, GHOST_TUserDataPtr C_void_ptr)
{
  ...
  if (type == GHOST_kEventQuitRequest) {
    ...
    if (ghostwin && GHOST_ValidWindow(g_system, ghostwin)) {
      ...
      win = (wm && wm->runtime) ? wm->runtime->winactive : nullptr;
    }
...
#elif defined(WITH_APPLE_CROSSPLATFORM) && defined(WITH_METAL_BACKEND)
...
bool WM_window_is_main_top_level(const wmWindow *win)
{
  ...
  if ((win->parent != nullptr) || screen == nullptr || screen->temp) {
    return false;
  }
  return true;
}
