                              ...
                              const wmFileReadPost_Params *params)
{
  ...
  if (use_data) {
    if (!G.background) {
      ...
      /* Undo type polls use #CTX_data_scene / #CTX_data_view_layer; after file-read the window is
       * set above but file data context can still be unset (embedded / minimal startup paths). */
      {
        wmWindow *win_undo = static_cast<wmWindow *>(wm->windows.first);
        if (win_undo) {
          Scene *scene_undo = WM_window_get_active_scene(win_undo);
          if (scene_undo == nullptr) {
            scene_undo = static_cast<Scene *>(bmain->scenes.first);
          }
          if (scene_undo) {
            CTX_data_scene_set(C, scene_undo);
          }
        }
      }
  ...
  wm_ios_ensure_single_window(C, wm, true);
#endif
...
static void wm_read_callback_pre_wrapper(bContext *C, const char *filepath)
{
  ...
  Main *bmain = CTX_data_main(C);
  if (bmain != nullptr) {
    BKE_callback_exec_string(bmain, BKE_CB_EVT_LOAD_PRE, filepath);
  }
}
...
bool WM_file_read(bContext *C,
                  ...
                  ReportList *reports)
{
  if (UNLIKELY((filepath == nullptr) || (filepath[0] == '\0'))) {
    BKE_report(reports, RPT_ERROR, "Skipped loading because file path is empty");
    return false;
  }
...
/* UNUSED, keep as this may be needed later & the comment below isn't self evident. */
  ...
  if (filepath_startup_override && filepath_startup_override[0] == '\0') {
    filepath_startup_override = nullptr;
  }
  ...
  if (success && bmain) {
    fprintf(stderr,
            "[ios] wm_homefile_read: embedded_startup_blob=%d path_attempt=\"%s\" screens=%d\n",
            int(loaded_factory_settings),
            filepath_startup[0] != '\0' ? filepath_startup : "",
            BLI_listbase_count(&bmain->screens));
    fflush(stderr);
  }
#endif
...
static wmOperatorStatus wm_homefile_read_exec(bContext *C, wmOperator *op)
{
  ...
  if (use_splash && WM_operatortype_find("WM_OT_splash", false) != nullptr) {
    WM_init_splash(C);
  }
...
static void wm_block_file_close_save(bContext *C, void *arg_block, void *arg_data)
{
  ...
  int modified_images_count = ED_image_save_all_modified_info(bmain, nullptr);
...
static uiBlock *block_create__close_file_dialog(bContext *C, ARegion *region, void *arg1)
{
  ...
  /* Filename. */
  const char *blendfile_path = (bmain != nullptr) ? BKE_main_blendfile_path(bmain) : "";
  char filename[FILE_MAX];
  if (blendfile_path[0] != '\0') {
    BLI_path_split_file_part(blendfile_path, filename, sizeof(filename));
  }
