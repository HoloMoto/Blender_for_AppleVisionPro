...
/**
 * After #ED_screens_init, ensure Layout is active and sidebars exist. Do not scan all screens in
 * Main for the "richest" layout — that incorrectly selects the Scripting workspace.
 */
static void wm_ios_finalize_startup_layout(bContext *C)
{
  ...
  if (win == nullptr || win->workspace_hook == nullptr) {
    return;
  }
...
void WM_init(bContext *C, int argc, const char **argv)
{
  ...
  GPU_backend_type_selection_set_override(GPU_BACKEND_METAL);
  /* Override alone does not set #g_backend_type; without detect, #GPU_backend_supported stays
   * false (default selection is OpenGL, which is not built on Apple). */
...
#endif
  /* Keep startup UI data on iOS as well.
   * `use_empty_data=true` strips most IDs after startup read and can leave windows without active
   * scene/workspace/screen, causing a permanent gray frame with no UI initialization. */
...
#if (defined(__APPLE__) && defined(TARGET_OS_IPHONE) && TARGET_OS_IPHONE) || \
    defined(WITH_APPLE_CROSSPLATFORM)
  /* `use_userdef == false` skips installing factory #UserDef from #wm_homefile_read_ex, but
   * #WM_check inside that read still creates GHOST windows and queries theme colors (#TH_BACK)
   * before #UI_init — leaving `U.themes` empty triggers asserts in #UI_ThemeGetColorPtr. */
  if (BLI_listbase_is_empty(&U.themes)) {
    UserDef *userdef_defaults = BKE_blendfile_userdef_from_defaults();
    BKE_blender_userdef_data_set_and_free(userdef_defaults);
  }
#endif
  ...
  /* Bootstrap uses `use_userdef=false`, so #wm_init_userdef (and #BKE_tempdir_init) are skipped.
   * Temp paths are still required for autosave, USD texture import, undo, etc. */
  if (!read_homefile_params.use_userdef) {
...
#endif
    /* Backend is created on first #GPU_context_create inside #WM_init_gpu; do not call
     * #GPU_render_begin before that or Metal/GL asserts fire in debug builds. */
    WM_init_gpu();
  ...
  /* Key-map initialization depends on this flag being true.
   * On iOS, setting this false leaves all default keymaps empty forever. */
...
#if defined(WITH_APPLE_CROSSPLATFORM) && defined(WITH_METAL_BACKEND)
  /* Homefile/user preferences can override GPU backend. Re-assert Metal on iOS
   * before any late window checks potentially create ghost windows. */
  GPU_backend_type_selection_set_override(GPU_BACKEND_METAL);
  GPU_backend_type_selection_detect();
#endif
  WM_check(C);
  ...
  if (!G.background) {
    Main *ios_bmain = CTX_data_main(C);
    if (ios_bmain == nullptr) {
      ios_bmain = G_MAIN;
      if (ios_bmain != nullptr) {
        CTX_data_main_set(C, ios_bmain);
      }
    }
    wmWindowManager *ios_wm = ios_bmain ?
                                  static_cast<wmWindowManager *>(ios_bmain->wm.first) :
                                  nullptr;
    if (ios_wm == nullptr && G_MAIN != nullptr) {
      ios_wm = static_cast<wmWindowManager *>(G_MAIN->wm.first);
      if (ios_wm != nullptr) {
        CTX_wm_manager_set(C, ios_wm);
      }
    }
    if (ios_bmain && ios_wm) {
      fprintf(stderr,
              "[ios] post-WM_check ED_screens_init bmain=%p scenes=%d workspaces=%d screens=%d wm=%p windows=%d\n",
              (void *)ios_bmain,
              BLI_listbase_count(&ios_bmain->scenes),
              BLI_listbase_count(&ios_bmain->workspaces),
              BLI_listbase_count(&ios_bmain->screens),
              (void *)ios_wm,
              BLI_listbase_count(&ios_wm->windows));
      fflush(stderr);
      /* After the final #WM_check, #ED_screens_init is skipped when #WM_INIT_FLAG_WINDOW is
       * already set — but embedded startup may still need scene/workspace wiring before the first
       * Metal frame runs event handlers. */
      ED_screens_init(C, ios_bmain, ios_wm);
#  if defined(__APPLE__) && defined(TARGET_OS_IPHONE) && TARGET_OS_IPHONE
      wm_ios_finalize_startup_layout(C);
#  endif
    }
  }
#endif
  wmWindowManager *wm_after_post = CTX_wm_manager(C);
  const int wm_window_count_after_post = wm_after_post ? BLI_listbase_count(&wm_after_post->windows) :
                                                         -1;
  wmWindow *first_win = (wm_after_post && !BLI_listbase_is_empty(&wm_after_post->windows)) ?
                            static_cast<wmWindow *>(wm_after_post->windows.first) :
                            nullptr;
  fprintf(stderr,
          "[ios] WM_init exit wm=%p windows=%d first_win=%p ghostwin=%p\n",
          (void *)wm_after_post,
          wm_window_count_after_post,
          (void *)first_win,
          first_win ? first_win->ghostwin : nullptr);
  fflush(stderr);
}
...
{
#ifdef WITH_APPLE_CROSSPLATFORM
  /* iOS startup is currently fragile when invoking splash as an operator while window/context
   * wiring is still in progress. Skip splash entirely on this platform. */
  return;
#endif
...
void WM_init_splash(bContext *C)
{
  ...
  if (WM_operatortype_find("WM_OT_splash", false) == nullptr) {
    return;
  }
...
#ifdef WITH_APPLE_CROSSPLATFORM
#  ifdef WITH_PYTHON
  /* iOS skips addon sync in #wm_file_read_post; enable factory default add-ons here so
   * Cycles and other bundled `addons_core` modules register render engines. */
  const char *imports[] = {"bpy", nullptr};
  BPY_run_string_eval(C, imports, "bpy.utils.load_scripts_extensions()");
  fprintf(stderr, "[ios] load_scripts_extensions done\n");
  fflush(stderr);
#  else
  UNUSED_VARS(C);
#  endif
