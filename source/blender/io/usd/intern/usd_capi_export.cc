...
static bool perform_usdz_conversion(const ExportJobData *data)
{
  ...
  if (!BLI_dir_create_recursive(usdc_temp_dir)) {
    BKE_reportf(data->params.worker_status->reports,
                RPT_ERROR,
                "USD Export: Couldn't create temporary directory %s",
                usdc_temp_dir);
    return false;
  }
  ...
  if (!BLI_is_file(usdz_temp_full_path)) {
    BKE_reportf(data->params.worker_status->reports,
                RPT_ERROR,
                "USD Export: USDZ packaging failed for %s",
                usdz_temp_full_path);
    return false;
  }
...
static void export_startjob(void *customdata, wmJobWorkerStatus *worker_status)
{
...

  {
    ...
    BLI_path_split_dir_part(data->unarchived_filepath, export_dir, sizeof(export_dir));
    if (export_dir[0] != '\0') {
      BLI_dir_create_recursive(export_dir);
    }
  }
