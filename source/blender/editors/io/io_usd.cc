...
bool ED_usd_export_operator_to_path(bContext *C, wmOperator *op, const char *filepath)
{
  RNA_string_set(op->ptr, "filepath", filepath);

  if (op->customdata != nullptr) {
    static_cast<USDOperatorOptions *>(op->customdata)->as_background_job = false;
  }

  const wmOperatorStatus retval = op->type->exec(C, op);

  /* #wm_usd_export_exec frees customdata; restore for a later file-select exec. */
  if (op->customdata == nullptr) {
    USDOperatorOptions *options = MEM_callocN<USDOperatorOptions>("USDOperatorOptions");
    options->as_background_job = true;
    op->customdata = options;
  ...
  return (retval & OPERATOR_FINISHED) && BLI_exists(filepath);
}
