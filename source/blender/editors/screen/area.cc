...
static void area_calc_totrct(const bScreen *screen, ScrArea *area, const rcti *window_rect)
{
  ...
  if (screen != nullptr && !ED_area_is_global(area) && screen->state != SCREENFULL &&
      !(screen->temp) &&
...
ScrArea *ED_screen_areas_iter_first(const wmWindow *win, const bScreen *screen)
{
  ...
  if (screen == nullptr) {
    /* Only global areas exist without a layout screen (incomplete UI state during startup). */
    for (ScrArea *a = global_area; a; a = a->next) {
      if (a->global && (a->global->flag & GLOBAL_AREA_IS_HIDDEN) == 0) {
        return a;
      }
    }
    return nullptr;
