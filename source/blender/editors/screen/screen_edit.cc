...
static int ios_area_center_x(const ScrArea *area)
{
  return (area->v1->vec.x + area->v3->vec.x) / 2;
}

static int ios_screen_center_x(const bScreen *screen)
{
  ScrVert *v1 = static_cast<ScrVert *>(screen->vertbase.first);
  ScrVert *v3 = static_cast<ScrVert *>(screen->vertbase.last);
  if (v1 == nullptr || v3 == nullptr) {
    return 0;
  }
  return (v1->vec.x + v3->vec.x) / 2;
}

static ScrArea *ios_find_largest_view3d_area(bScreen *screen)
{
  ScrArea *best = nullptr;
  int best_px = 0;
  LISTBASE_FOREACH (ScrArea *, area, &screen->areabase) {
    if (area->global || area->spacetype != SPACE_VIEW3D) {
      continue;
    }
    const int px = int(area->winx) * int(area->winy);
    if (px > best_px) {
      best_px = px;
      best = area;
    }
...
static void ios_unhide_view3d_tool_regions(bContext *C, ScrArea *v3d)
{
  if (v3d == nullptr || v3d->spacetype != SPACE_VIEW3D) {
    return;
  }
  LISTBASE_FOREACH (ARegion *, region, &v3d->regionbase) {
    if (!ELEM(region->regiontype, RGN_TYPE_TOOLS, RGN_TYPE_UI)) {
      continue;
    }
    if (region->flag & RGN_FLAG_HIDDEN) {
      region->flag &= ~RGN_FLAG_HIDDEN;
      ED_region_visibility_change_update_ex(C, v3d, region, false, true);
    }
    if (region->sizex == 0 && region->regiontype == RGN_TYPE_TOOLS) {
      region->sizex = UI_TOOLBAR_WIDTH;
    }
...
/**
 * Factory Layout on iOS: Outliner on the right, Properties stacked below it.
 * Only repair missing/broken pieces — never move Outliner to the left or fold Properties
 * into the 3D-view N-panel.
 */
static bool ios_area_on_right(const ScrArea *area, const int screen_cx)
{
  return ios_area_center_x(area) > screen_cx;
}

static void ios_close_left_outliner_legacy(bContext *C, bScreen *screen, const int screen_cx)
{
  LISTBASE_FOREACH_MUTABLE (ScrArea *, area, &screen->areabase) {
    if (area->global) {
      continue;
    }
    if (area->spacetype == SPACE_OUTLINER && !ios_area_on_right(area, screen_cx)) {
      fprintf(stderr, "[ios] removed legacy left-side Outliner area\n");
      fflush(stderr);
      screen_area_close(C, nullptr, screen, area);
    }
  }
}

static bool ios_ensure_right_outliner_and_properties(
    bContext *C, wmWindow *win, bScreen *screen, ScrArea *v3d)
{
  if (v3d == nullptr || v3d->winx <= 2 * AREAMINX * U.scale_factor) {
    return false;
  }

  /* ~22% right column: Outliner on top, Properties below (factory Layout). */
  const float right_frac = 0.22f;
  ScrArea *right_col = area_split(win, screen, v3d, SCREEN_AXIS_V, 1.0f - right_frac, false);
  if (right_col == nullptr) {
    return false;
  }

  ScrArea *properties = area_split(win, screen, right_col, SCREEN_AXIS_H, 0.45f, false);
  if (properties != nullptr) {
    ED_area_newspace(C, right_col, SPACE_OUTLINER, false);
    ED_area_newspace(C, properties, SPACE_PROPERTIES, false);
  }
  else {
    ED_area_newspace(C, right_col, SPACE_OUTLINER, false);
  }

  ED_area_tag_redraw(right_col);
  if (properties != nullptr) {
    ED_area_tag_redraw(properties);
  }
  ED_area_tag_redraw(v3d);
  return true;
}

/** File-read / multi-window glitches can squash TOPBAR headers to zero height (World menu vanishes). */
static void ios_fix_broken_headers(wmWindow *win, bScreen *screen)
{
  bool fixed = false;
  ED_screen_areas_iter (win, screen, area) {
    LISTBASE_FOREACH (ARegion *, region, &area->regionbase) {
      ...
      if (BLI_rcti_size_y(&region->winrct) <= 1 || region->sizey <= 1) {
        region->sizey = HEADERY;
        region->flag &= ~(RGN_FLAG_TOO_SMALL | RGN_FLAG_HIDDEN);
        ED_region_tag_redraw(region);
        fixed = true;
      }
    }
  ...
  if (fixed) {
    ...
    screen->do_draw = true;
    fprintf(stderr, "[ios] fixed zero-height header regions\n");
    fflush(stderr);
  }
}
#endif

void ED_screens_ios_ensure_sidebar_editors(bContext *C, Main *bmain, wmWindow *win)
{
#if !defined(__APPLE__) || !defined(TARGET_OS_IPHONE) || !TARGET_OS_IPHONE
  UNUSED_VARS(C, bmain, win);
  return;
#else
  if (C == nullptr || bmain == nullptr || win == nullptr) {
    return;
  }
  ...
  const int screen_cx = ios_screen_center_x(screen);
  bool has_right_outliner = false;
  bool has_right_properties = false;
  ...
  LISTBASE_FOREACH (ScrArea *, area, &screen->areabase) {
    ...
    if (area->spacetype == SPACE_OUTLINER && ios_area_on_right(area, screen_cx)) {
      has_right_outliner = true;
    }
    if (area->spacetype == SPACE_PROPERTIES && ios_area_on_right(area, screen_cx)) {
      has_right_properties = true;
    }
  }
  ...
  /* Undo previous bootstrap that forced Outliner onto the left. */
  ios_close_left_outliner_legacy(C, screen, screen_cx);

  ScrArea *v3d = ios_find_largest_view3d_area(screen);
  if (v3d != nullptr) {
    ios_unhide_view3d_tool_regions(C, v3d);
  }

  if (!has_right_outliner || !has_right_properties) {
    ScrArea *right_outliner = nullptr;
    ScrArea *right_properties = nullptr;
    LISTBASE_FOREACH (ScrArea *, area, &screen->areabase) {
      if (area->global) {
        continue;
      }
      ...
      if (area->spacetype == SPACE_PROPERTIES && ios_area_on_right(area, screen_cx)) {
        right_properties = area;
      }
    }

    if (!has_right_outliner && !has_right_properties) {
      if (ios_ensure_right_outliner_and_properties(C, win, screen, v3d)) {
        has_right_outliner = true;
        has_right_properties = true;
        fprintf(stderr, "[ios] created right column: Outliner + Properties\n");
        fflush(stderr);
      }
    }
    else if (!has_right_properties && right_outliner != nullptr) {
      ScrArea *properties = area_split(
          win, screen, right_outliner, SCREEN_AXIS_H, 0.45f, false);
      if (properties != nullptr) {
        ED_area_newspace(C, properties, SPACE_PROPERTIES, false);
        ED_area_tag_redraw(properties);
        has_right_properties = true;
        fprintf(stderr, "[ios] added Properties below Outliner\n");
        fflush(stderr);
      }
    }
    else if (!has_right_outliner && right_properties != nullptr) {
      ScrArea *outliner = area_split(
          win, screen, right_properties, SCREEN_AXIS_H, 0.55f, false);
      if (outliner != nullptr) {
        /* fac > 0.5: new area is top (Outliner), right_properties stays bottom. */
        ED_area_newspace(C, outliner, SPACE_OUTLINER, false);
        ED_area_tag_redraw(outliner);
        has_right_outliner = true;
        fprintf(stderr, "[ios] added Outliner above Properties\n");
        fflush(stderr);
      }
    }
  }

  ios_fix_broken_headers(win, screen);

  if (has_right_outliner && has_right_properties) {
    ED_screen_refresh(C, CTX_wm_manager(C), win);
    fprintf(stderr, "[ios] layout ok: Outliner(right) Properties(right)\n");
    fflush(stderr);
    return;
  }
  ...
  fprintf(stderr,
          "[ios] layout: Outliner(right)=%d Properties(right)=%d\n",
          int(has_right_outliner),
          int(has_right_properties));
  fflush(stderr);
#endif
}

void ED_screens_ios_activate_layout_workspace(bContext *C, Main *bmain, wmWindow *win)
{
#if !defined(__APPLE__) || !defined(TARGET_OS_IPHONE) || !TARGET_OS_IPHONE
  UNUSED_VARS(C, bmain, win);
  return;
#else
  if (C == nullptr || bmain == nullptr || win == nullptr || win->workspace_hook == nullptr) {
    return;
  }

  WorkSpace *layout_ws = static_cast<WorkSpace *>(
      BLI_findstring(&bmain->workspaces, "Layout", offsetof(ID, name) + 2));
  if (layout_ws == nullptr) {
    return;
  }

  WorkSpace *active = BKE_workspace_active_get(win->workspace_hook);
  if (active == layout_ws) {
    return;
  }

  /* After an explicit file reload the startup screen is already valid — do not override the
   * user's workspace (e.g. Shading / World properties) unless the screen is broken. */
  bScreen *active_screen = WM_window_get_active_screen(win);
  if (active != nullptr && active_screen != nullptr &&
      !BLI_listbase_is_empty(&active_screen->areabase))
  {
    return;
  }
  ...
  if (ED_workspace_change(layout_ws, C, wm, win)) {
    fprintf(stderr,
            "[ios] activated Layout workspace (was %s)\n",
            active ? active->id.name + 2 : "<none>");
    fflush(stderr);
...
void ED_screens_ios_ensure_factory_workspaces(bContext *C, Main *bmain, wmWindow *win)
{
#if !defined(__APPLE__) || !defined(TARGET_OS_IPHONE) || !TARGET_OS_IPHONE
  UNUSED_VARS(C, bmain, win);
  return;
  ...
  const int workspace_count = BLI_listbase_count(&bmain->workspaces);
  if (workspace_count >= 8) {
    ED_screens_ios_activate_layout_workspace(C, bmain, win);
    return;
  }

  WorkSpace *base = static_cast<WorkSpace *>(
      BLI_findstring(&bmain->workspaces, "Layout", offsetof(ID, name) + 2));
  if (base == nullptr) {
    base = BKE_workspace_active_get(win->workspace_hook);
  }
  if (base == nullptr) {
    base = static_cast<WorkSpace *>(bmain->workspaces.first);
  }
  if (base == nullptr) {
    return;
  }
  ...
  int added = 0;
  for (const char **name_p = default_names; *name_p != nullptr; name_p++) {
    const char *name = *name_p;
    if (BLI_findstring(&bmain->workspaces, name, offsetof(ID, name) + 2)) {
      continue;
    }

    WorkSpace *workspace_new = ED_workspace_duplicate(base, bmain, win);
    BKE_libblock_rename(*bmain, workspace_new->id, name);
    added++;
  }
  ...
  fprintf(stderr,
          "[ios] workspaces: had=%d added=%d total=%d (top-bar tabs)\n",
          workspace_count,
          added,
          BLI_listbase_count(&bmain->workspaces));
  fflush(stderr);
...
void ED_screen_do_listen(bContext *C, const wmNotifier *note)
{
  ...
  switch (note->category) {
    case NC_WM:
      if (note->data == ND_FILEREAD && screen != nullptr) {
        screen->do_draw = true;
      }
      break;
    case NC_WINDOW:
      ...
      break;
    case NC_SCREEN:
      if (screen != nullptr && note->action == NA_EDITED) {
        if (WM_capabilities_flag() & WM_CAPABILITY_WINDOW_DECORATION_STYLES) {
          WM_window_decoration_style_apply(win, screen);
        }
...
static void screen_refresh_if_needed(bContext *C, wmWindowManager *wm, wmWindow *win)
{
  ...
  if (did_full_refresh) {
    /* Ensure at least one frame is submitted after layout/region init.
     * On embedded startup paths there may be no notifier-driven redraw yet,
     * causing draw loop heartbeat without visible UI updates. */
    screen->do_draw = true;
  }
...
void ED_screens_init(bContext *C, Main *bmain, wmWindowManager *wm)
{
  ...
  LISTBASE_FOREACH (wmWindow *, win, &wm->windows) {
    ...
    /* Embedded/iOS startup: startup.blend may be missing from bundle or file-read may be stripped,
     * leaving Main without any UI/scene data. Create the minimal set so the window can initialize
     * a screen and draw at least one frame. */
    if (BLI_listbase_is_empty(&bmain->scenes)) {
      Scene *scene = BKE_scene_add(bmain, "Scene");
      if (scene) {
        win->scene = scene;
        ViewLayer *vl = BKE_view_layer_default_view(scene);
        if (vl) {
          STRNCPY_UTF8(win->view_layer_name, vl->name);
        }
      }
    }

    if (BLI_listbase_is_empty(&bmain->screens)) {
      /* Use current window size if available; fall back to a sane default. */
      const int sizex = (win->sizex > 0) ? win->sizex : 1920;
      const int sizey = (win->sizey > 0) ? win->sizey : 1080;
      rcti rect{};
      rect.xmin = 0;
      rect.ymin = 0;
      rect.xmax = sizex;
      rect.ymax = sizey;
      screen_add(bmain, "Screen", &rect);
    }

    if (BLI_listbase_is_empty(&bmain->workspaces) && !BLI_listbase_is_empty(&bmain->screens)) {
      WorkSpace *ws = BKE_workspace_add(bmain, "Workspace");
      if (ws) {
        bScreen *scr = static_cast<bScreen *>(bmain->screens.first);
        BKE_workspace_layout_add(bmain, ws, scr, "Layout");
      }
    }

    /* If fallback bootstrap created only an empty area, promote it to a usable editor so region
     * initialization can produce visible regions and trigger redraw logic. */
    if (!BLI_listbase_is_empty(&bmain->screens)) {
      bScreen *screen_first = static_cast<bScreen *>(bmain->screens.first);
      if (screen_first && BLI_listbase_is_single(&screen_first->areabase)) {
        ScrArea *area = static_cast<ScrArea *>(screen_first->areabase.first);
        if (area && area->spacetype == SPACE_EMPTY) {
          CTX_wm_area_set(C, area);
          ED_area_newspace(C, area, SPACE_VIEW3D, false);
          CTX_wm_area_set(C, nullptr);
          screen_first->do_refresh = true;
          screen_first->do_draw = true;
        }
      }
    }
    ...
    BKE_workspace_active_layout_ensure_for_window(bmain, win->workspace_hook, win->winid, workspace);
    workspace = BKE_workspace_active_get(win->workspace_hook);

    /* Active workspace may not contain any layout referencing startup screens (ordering / file
     * anomalies). Attach a usable main screen (prefer one with areas) or create a layout as a
     * last resort. */
    bScreen *active_screen = WM_window_get_active_screen(win);
    const bool active_screen_usable = (active_screen != nullptr &&
                                       !BLI_listbase_is_empty(&active_screen->areabase));
    if (!active_screen_usable && !BLI_listbase_is_empty(&bmain->screens)) {
      bScreen *any_screen = nullptr;
      LISTBASE_FOREACH (bScreen *, screen_iter, &bmain->screens) {
        if (!BLI_listbase_is_empty(&screen_iter->areabase)) {
          any_screen = screen_iter;
          break;
        }
      }
      if (any_screen == nullptr) {
        any_screen = static_cast<bScreen *>(bmain->screens.first);
      }
      WorkSpace *ws_match = nullptr;
      WorkSpaceLayout *layout_g = BKE_workspace_layout_find_global(bmain, any_screen, &ws_match);
      if (layout_g != nullptr && ws_match != nullptr) {
        BKE_workspace_active_set(win->workspace_hook, ws_match);
        BKE_workspace_active_layout_set(win->workspace_hook, win->winid, ws_match, layout_g);
      }
      else {
        workspace = BKE_workspace_active_get(win->workspace_hook);
        if (workspace != nullptr) {
          WorkSpaceLayout *new_layout = BKE_workspace_layout_add(bmain, workspace, any_screen, "Layout");
          if (new_layout != nullptr) {
            BKE_workspace_active_layout_set(
                win->workspace_hook, win->winid, workspace, new_layout);
          }
        }
      }
    }

    /* Final safety-net for embedded startup: if active screen is still empty, prefer the Layout
     * workspace screen (never the first rich screen in Main — often Scripting). */
    active_screen = WM_window_get_active_screen(win);
    if ((active_screen == nullptr || BLI_listbase_is_empty(&active_screen->areabase)) &&
        !BLI_listbase_is_empty(&bmain->screens))
    {
      WorkSpace *layout_ws = static_cast<WorkSpace *>(
          BLI_findstring(&bmain->workspaces, "Layout", offsetof(ID, name) + 2));
      if (layout_ws != nullptr) {
        LISTBASE_FOREACH (WorkSpaceLayout *, layout, &layout_ws->layouts) {
          bScreen *screen_iter = layout->screen;
          if (screen_iter == nullptr || BLI_listbase_is_empty(&screen_iter->areabase)) {
            continue;
          }
          BKE_workspace_active_set(win->workspace_hook, layout_ws);
          BKE_workspace_active_layout_set(win->workspace_hook, win->winid, layout_ws, layout);
          active_screen = screen_iter;
          break;
        }
      }
      if (active_screen == nullptr || BLI_listbase_is_empty(&active_screen->areabase)) {
        LISTBASE_FOREACH (bScreen *, screen_iter, &bmain->screens) {
          ...
          WorkSpace *ws_match = nullptr;
          WorkSpaceLayout *layout_g = BKE_workspace_layout_find_global(bmain, screen_iter, &ws_match);
          if (layout_g == nullptr || ws_match == nullptr) {
            continue;
          }
          /* Skip Scripting — it wins area-count heuristics but is not the desired default. */
          if (STREQ(ws_match->id.name + 2, "Scripting")) {
            continue;
          }
          BKE_workspace_active_set(win->workspace_hook, ws_match);
          BKE_workspace_active_layout_set(win->workspace_hook, win->winid, ws_match, layout_g);
    ...
    /* Embedded / stripped startup can leave #wmWindow.scene unset while workspace/screen exist. */
    ...
    ED_screens_ios_ensure_sidebar_editors(C, bmain, win);
    ED_screens_ios_ensure_factory_workspaces(C, bmain, win);
    ED_screens_ios_activate_layout_workspace(C, bmain, win);
  }
...
ScrArea *ED_screen_state_toggle(bContext *C, wmWindow *win, ScrArea *area, const short state)
{
  ...
  return static_cast<ScrArea *>(screen->areabase.first);
    ...
    TARGET_OS_IPHONE
static void ed_screen_temp_space_open_ios_redraw(bContext *C, ScrArea *area)
{
  if (area != nullptr) {
    ...
    bContext *C, const char *title, eSpace_Type space_type, int display_type, bool dialog)
{
  switch (display_type) {
...
#if defined(WITH_APPLE_CROSSPLATFORM) && defined(__APPLE__) && defined(TARGET_OS_IPHONE) && \
    TARGET_OS_IPHONE
        ed_screen_temp_space_open_ios_redraw(C, CTX_wm_area(C));
#endif
    ...
    defined(TARGET_OS_IPHONE) && TARGET_OS_IPHONE
            ed_screen_temp_space_open_ios_redraw(C, screen_area);
#endif
            return screen_area;
          }
        }
...
#if defined(WITH_APPLE_CROSSPLATFORM) && defined(__APPLE__) && defined(TARGET_OS_IPHONE) && \
    TARGET_OS_IPHONE
        ed_screen_temp_space_open_ios_redraw(C, area);
#endif
        return area;
      }
