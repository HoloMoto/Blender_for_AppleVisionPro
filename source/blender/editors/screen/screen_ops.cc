...
static wmOperatorStatus userpref_show_exec(bContext *C, wmOperator *op)
{
  ...
  /* Single-surface platform: open preferences fullscreen in the current window. */
  area = ED_screen_temp_space_open(
      C, nullptr, SPACE_USERPREF, USER_TEMP_SPACE_DISPLAY_FULLSCREEN, false);
  if (area != nullptr) {
    wmWindow *win = CTX_wm_window(C);
    wmWindowManager *wm = CTX_wm_manager(C);
