...
/**
 * iOS: Minimal/embedded startup may ship without Outliner or Properties areas.
 * When no outliner exists anywhere in Main, split the largest non-global 3D view to add them.
 * No-op on non-iOS platforms.
 */
void ED_screens_ios_ensure_sidebar_editors(bContext *C, Main *bmain, wmWindow *win);
/**
 * iOS: When embedded startup is invalid (Git LFS pointers), duplicate the active workspace so
 * the top-bar workspace tabs (Layout, UV Editing, Shading, ...) appear.
 */
void ED_screens_ios_ensure_factory_workspaces(bContext *C, Main *bmain, wmWindow *win);
/**
 * iOS: Activate the "Layout" workspace (factory default). Avoids ending up on Scripting because
 * richer-screen heuristics favor its outliner/text-editor areas.
 */
